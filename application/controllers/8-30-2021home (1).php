...
class Home extends CI_Controller {
	...
	function tracking($id=null){
		...
		if(isset($id)){
			...
			try{
		        $masterUrl = "http://track.ctracksystems.com/TrackingService.svc/express/TrackManifest/".$id;
		        $detailsUrl = "http://track.ctracksystems.com/TrackingService.svc/express/TrackDetails/".$id;

		        $masterData = $this->home_model->get_data($masterUrl);
		        $detailsData = $this->home_model->get_data($detailsUrl);

		        if($masterData != null && $detailsData != null){
		        	$master = json_decode($masterData);
			        $details = json_decode($detailsData);

			        if($master != null && $master->ReadExpressManifestDetailsResult){
			            
			          $manifest = $master->ReadExpressManifestDetailsResult[0];
			          //print_r($manifest);
			          //echo $manifest->CONSIGNEEADDRESS1;
			          		       
    			     $consignee = '<h2>'.$manifest->CONSIGNEECITY .'</h2>'.$manifest->CONISGNEE .', '.$manifest->CONSIGNEECOUNTRY .', '.$manifest->DESTINATION;

			          $shipper = '<h2>'.$manifest->SHIPPERCITY .'</h2>'.$manifest->SHIPPER .', '.$manifest->SHIPPERCOUNTRY .', '.$manifest->SHIPPERREFERENCE;

			          $result['awbNo'] = $manifest->AWBNO;
			          $result['origin'] = $shipper;
			          $result['destination'] = $consignee;
			         // $result['goods'] = $manifest->DESCRIPTIONOFGOODS;
			          $pickdate = strtotime($manifest->PICKUPDATE);
					  $pickdate = date('l, d M Y',$pickdate);

			          $result['pickup_date'] = $pickdate;
			          $result['weight'] = $manifest->WEIGHT;
			        ...
			        if($details != null && is_array($details->ReadExpressTrackDetailsResult)){
			        	
			           $result['trackSubDetails'] =  $details->ReadExpressTrackDetailsResult;
			        }
		        }
		      }
		      catch(Exception $e){
...
// 			try{
// 		        $masterUrl = "http://express.iship.co.in/iSocketService.svc/TrackManifest/".$id;
// 		        $detailsUrl = "http://express.iship.co.in/iSocketService.svc/TrackDetails/".$id;
...
// 			        if($master != null && $master->ReadManifestDetailsResult){
// 			          $manifest = $master->ReadManifestDetailsResult[0];
			          		          
// 			          $consignee = '<h2>'.$manifest->CONSIGNEECITY .'</h2>'.$manifest->CONISGNEE .', '.$manifest->CONSIGNEEADDRESS1 .', '.$manifest->CONSIGNEEADDRESS2;

// 			          $shipper = '<h2>'.$manifest->SHIPPERCITY .'</h2>'.$manifest->SHIPPER .', '.$manifest->SHIPPERADDRESS1 .', '.$manifest->SHIPPERADDRESS2;
...
// 			        if($details != null && is_array($details->ReadTrackDetailsResult)){
			        	
// 			           $result['trackSubDetails'] =  $details->ReadTrackDetailsResult;
// 			        }
// 		        }
// 		      }
// 		      catch(Exception $e){
...
// 		}
        ...
        $this->load->view('includes/header',$result);
		$this->load->view('tracking',$result);
		$this->load->view('includes/footer');
	}
